...
from pdf2gtfs.datastructures.table.fields2 import (
    EmptyDataField, EmptyField,
    F, Fs, OF)
from pdf2gtfs.datastructures.table.linked_list2 import (
    Direction,
    E, H, Orientation, QuadLinkedList, S, V, W)
...
class Table(QuadLinkedList[F, OF]):
    ...
    def insert_empty_fields_from_map(
        ...
        for ref_field in ref_fields:
            field = fields[i]
            if ref_field.is_overlap(o, field, 0.9):
                i += 1
                continue
            e = EmptyField()
    ...
    def expand_west(self, fields: Fs) -> bool:
        ...
        first_col = list(next(self.col(self.left)).iter(S))
        adjacent_fields = WBounds.select_adjacent_fields(
            list(first_col), fields)
        if not adjacent_fields:
            return False
        # Remove fields, that would require a new row.
        starter_id = 0
        for adj_field in adjacent_fields:
            for i, field in enumerate(first_col[starter_id:], starter_id):
                if adj_field.is_overlap(V, field, 0.9):
                    break
            else:
        ...
        link_nodes(S, adjacent_fields)
        head = self.insert_empty_fields_from_map(V, first_col, adjacent_fields)
        tail = list(head.iter(S))[-1]
        new_col = Table(head, tail)

        self.insert(W, first_col[0], new_col.top)
        return True
